...
def update_water_bomb(bomb, bounds_height, is_in_map_func=None):
    ...
    if is_in_map_func is None:
        bomb['x'] = new_x
        bomb['y'] = new_y
    else:
        r = bomb['collision_radius']

    # tenta mover nos dois eixos
        if is_in_map_func(new_x, new_y, r):
            bomb['x'] = new_x
            bomb['y'] = new_y
        else:
        # tenta separar os movimentos (boa prática)
            if is_in_map_func(new_x, bomb['y'], r):
                bomb['x'] = new_x
            if is_in_map_func(bomb['x'], new_y, r):
                bomb['y'] = new_y
